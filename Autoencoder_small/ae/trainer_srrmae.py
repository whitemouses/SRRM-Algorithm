...
def _srrm_distance1(encoded_samples, distribution_samples, p=2, device='cpu'):
...
def SRRM_distance(encoded_samples, distribution_fn=rand_cirlce2d, p=2, device='cpu'):

    batch_size = encoded_samples.size(0)
    z = distribution_fn(batch_size).to(device)
    hpd = _srrm_distance1(encoded_samples, z, p, device)  # _new_python1
    return hpd


class SRRMAEBatchTrainer:
    ...
    def eval_on_batch(self, x):
        ...
        _swd = SRRM_distance(z, self._distribution_fn, self.p_, self._device)
