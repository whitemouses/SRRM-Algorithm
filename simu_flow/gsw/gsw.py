...
class GSW():
    ...
    def gsw(self,X,Y,p=2,theta=None):
        ...
        if self.ftype=='BSP':
            ...
            X_np = X.detach().to("cpu").contiguous().double().numpy()
            Y_np = Y.detach().to("cpu").contiguous().double().numpy()

            plan, cost_bsp = base.computeBijectiveBSPOT(X_np, Y_np, nb_plans=10)

            return torch.pow(torch.mean((X - Y[plan]) ** p), 1 / p)
        elif self.ftype == 'HCP':
            ...
            return torch.pow(torch.mean((X[Xr, :] - Y[Yr, :]) ** p), 1 / p)

        elif self.ftype == 'SRRM':
            X_np = X.detach().to("cpu").contiguous().double().numpy()
            Y_np = Y.detach().to("cpu").contiguous().double().numpy()

            plan, hist = base.iter_match_dpq(
                X_np, Y_np,
                num_rounds=10,
                z_mode="uniform01",
                verbose=False,
    ...
    def max_gsw(self,X,Y,iterations=50,lr=1e-4):
        ...
        for i in range(iterations):
            optimizer.zero_grad()
            loss=-self.gsw(X.to(self.device),Y.to(self.device),theta=self.theta.to(self.device))
            total_loss[i]=loss.item()
            loss.backward(retain_graph=True)
            optimizer.step()
            self.theta.data/=torch.sqrt(torch.sum(self.theta.data**2))

        return self.gsw(X.to(self.device),Y.to(self.device),theta=self.theta.to(self.device))
