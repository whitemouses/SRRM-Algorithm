...
def sinkhorn_w2(x, y, eps=0.1, niter=100, tol=1e-9):
    x = np.asarray(x)
    y = np.asarray(y)
    ...
    w2_reg_sq = ot.bregman.sinkhorn2(
        a, b, M,
        reg=eps,
        numItermax=niter,
        stopThr=tol,
    )
    # sinkhorn2 返回的是“距离平方”，外面开根号
    return np.sqrt(w2_reg_sq)


def _sinkhorn_w2_sq(x, y, eps=0.1, niter=100, tol=1e-9):
    x = np.asarray(x)
    y = np.asarray(y)
...
def sinkhorn_div_w2(x, y, eps=0.1, niter=100, tol=1e-9):
    # W_{ε}^2(x,y)
    w_xy = _sinkhorn_w2_sq(x, y, eps, niter, tol)
    # W_{ε}^2(x,x)
    w_xx = _sinkhorn_w2_sq(x, x, eps, niter, tol)
    # W_{ε}^2(y,y)
    w_yy = _sinkhorn_w2_sq(y, y, eps, niter, tol)

    sd2 = w_xy - 0.5 * w_xx - 0.5 * w_yy
    # 理论上应该是 ≥0，数值上可能有一点点负，保护一下
    sd2 = max(sd2, 0.0)
    return np.sqrt(sd2)
...
def main():
    ...
    for run in range(n_runs):
        ...
        for i in range(Ktrain):
            for j in range(i + 1, Ktrain):
                ...
                hcp_dist_matrix[i, j] =  sinkhorn_w2(train_3D[i],  train_3D[j], eps=eps, niter=niter, tol=tol)
        hcp_dist_matrix = hcp_dist_matrix + hcp_dist_matrix.T
        t1 = time.time()
        dt_train_train = t1 - t0
        print("Time for train-train Sinkhorn distances:", dt_train_train, "seconds")
        ...
        for i in range(Ktest):
            for j in range(Ktrain):
                hcp_dist_matrix2[i, j] = sinkhorn_w2(test_3D[i], train_3D[j], eps=eps, niter=niter, tol=tol)
        t3 = time.time()
        dt_test_train = t3 - t2
        print("Time for test-train Sinkhorn distances:", dt_test_train, "seconds")
    ...
    print("Train-train Sinkhorn time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
        min(time_train_train_list), max(time_train_train_list), sum(time_train_train_list) / len(time_train_train_list)
    ))

    print("Test-train Sinkhorn time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
