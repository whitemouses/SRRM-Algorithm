...
def main():
    ...
    for run in range(n_runs):
        ...
        for i in range(Ktrain):
            for j in range(i + 1, Ktrain):
                hcp_dist_matrix[i, j] = ot.sliced.sliced_wasserstein_distance(train_3D[i], train_3D[j], seed=2022,n_projections=10)

        hcp_dist_matrix = hcp_dist_matrix + hcp_dist_matrix.T
        t1 = time.time()
        dt_train_train = t1 - t0
        print("Time for train-train SW distances:", dt_train_train, "seconds")
        ...
        for i in range(Ktest):
            for j in range(Ktrain):
                hcp_dist_matrix2[i, j] = ot.sliced.sliced_wasserstein_distance(test_3D[i], train_3D[j], seed=2022,n_projections=10)
        t3 = time.time()
        dt_test_train = t3 - t2
        print("Time for test-train SW distances:", dt_test_train, "seconds")
    ...
    print("Train-train SW time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
        min(time_train_train_list), max(time_train_train_list), sum(time_train_train_list) / len(time_train_train_list)
    ))

    print("Test-train SW time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
