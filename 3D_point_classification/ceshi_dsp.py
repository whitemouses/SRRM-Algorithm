...
def main():
    ...
    for run in range(n_runs):
        ...
        for i in range(Ktrain):
            for j in range(i + 1, Ktrain):
                ...
                hcp_dist_matrix[i, j] = np.sqrt(cost_bsp)

        hcp_dist_matrix = hcp_dist_matrix + hcp_dist_matrix.T
        t1 = time.time()
        dt_train_train = t1 - t0
        print("Time for train-train BSP distances:", dt_train_train, "seconds")
        ...
        for i in range(Ktest):
            for j in range(Ktrain):
                # _, avg_cost =  base.computeBijectiveBSPOT_GR(test_3D[i], train_3D[j],  nb_plans=10, guided_depth=2, n_dirs=10, k_cross=8, R=4)
                X = test_3D[i]
                Y = train_3D[j]

                plan_bsp, cost_bsp = base.computeBijectiveBSPOT(X, Y,5)
                hcp_dist_matrix2[i, j] = np.sqrt(cost_bsp)
        t3 = time.time()
        dt_test_train = t3 - t2
        print("Time for test-train BSP distances:", dt_test_train, "seconds")
    ...
    print("Train-train BSP time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
        min(time_train_train_list), max(time_train_train_list), sum(time_train_train_list) / len(time_train_train_list)
    ))

    print("Test-train BSP time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
