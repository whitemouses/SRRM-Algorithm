...
def main():
    ...
    for run in range(n_runs):
        ...
        for i in range(Ktrain):
            for j in range(i + 1, Ktrain):
                ...
                cost_bsp = ((X - Y[plan]) ** 2).sum() / X.shape[0]


        hcp_dist_matrix = hcp_dist_matrix + hcp_dist_matrix.T
        t1 = time.time()
        dt_train_train = t1 - t0
        print("Time for train-train SRRM distances:", dt_train_train, "seconds")
        ...
        t3 = time.time()
        dt_test_train = t3 - t2
        print("Time for test-train SRRM distances:", dt_test_train, "seconds")
    ...
    print("Train-train SRRM time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
        min(time_train_train_list), max(time_train_train_list), sum(time_train_train_list) / len(time_train_train_list)
    ))

    print("Test-train SRRM time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
