...
def main():
    ...
    for run in range(n_runs):
        ...
        print("Time for train-train TCP distances:", dt_train_train, "seconds")
        ...
        t3 = time.time()
        dt_test_train = t3 - t2
        print("Time for test-train TCP distances:", dt_test_train, "seconds")
        ...
        # for i in range(Ktrain):
        #     for j in range(i + 1, Ktrain):
        #         hcp_dist_matrix[i, j] = HCP(train_3D[i], train_3D[j], 'Hm', k=1)
        # hcp_dist_matrix = hcp_dist_matrix + hcp_dist_matrix.T
        # # ---------- 2) 测试–训练 HCP 距离矩阵 ----------
        # hcp_dist_matrix2 = np.zeros((Ktest, Ktrain))
        # for i in range(Ktest):
        #     for j in range(Ktrain):
        #         hcp_dist_matrix2[i, j] = HCP(test_3D[i], train_3D[j], 'Hm', k=1)
        # # ---------- 3) KNN 拟合 + 预测 ----------
        # estimator1 = KNeighborsClassifier(metric='precomputed', n_neighbors=5)
        # estimator1.fit(hcp_dist_matrix, train_label)
        #
        # y_pred = estimator1.predict(hcp_dist_matrix2)
        # acc1 = np.sum(y_pred == test_label) / Ktest
        # print('Accuracy HCP is ', acc1, '%')
    ...
    print("Train-train TCP time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
        min(time_train_train_list), max(time_train_train_list), sum(time_train_train_list) / len(time_train_train_list)
    ))

    print("Test-train TCP time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
