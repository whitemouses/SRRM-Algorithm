...
def im_kernel_sum_np_repeat(z1, z2, Cs, exclude_diag=True):
    z1 = np.asarray(z1, dtype=np.float64)
    z2 = np.asarray(z2, dtype=np.float64)
    assert z1.shape == z2.shape and z1.ndim == 2

    n, d = z1.shape
    z11 = np.repeat(z1[:, None, :], n, axis=1)  # (n,n,d)
    z22 = np.repeat(z2[None, :, :], n, axis=0)  # (n,n,d)

    dist2 = np.sum((z11 - z22) ** 2, axis=2)    # (n,n)
...
def mmd_imq_dist_np(X, Y, Cs):
    ...
    mmd2 = (
        im_kernel_sum_np_repeat(X, X, Cs, exclude_diag=True) / (n * (n - 1)) +
        im_kernel_sum_np_repeat(Y, Y, Cs, exclude_diag=True) / (n * (n - 1)) -
        2.0 * im_kernel_sum_np_repeat(X, Y, Cs, exclude_diag=False) / (n * n)
    )
    return np.float64(np.sqrt(max(float(mmd2), 0.0)))
...
def main():
    ...
    for run in range(n_runs):
        ...
        for i in range(Ktrain):
            for j in range(i + 1, Ktrain):
                # hcp_dist_matrix[i, j] = HCP(train_3D[i], train_3D[j], 'Hm')
                hcp_dist_matrix[i, j] = mmd_imq_dist_np(train_3D[i],  train_3D[j],  Cs)
        hcp_dist_matrix = hcp_dist_matrix + hcp_dist_matrix.T
        t1 = time.time()
        dt_train_train = t1 - t0
        print("Time for train-train MMD distances:", dt_train_train, "seconds")
        ...
        for i in range(Ktest):
            for j in range(Ktrain):
                # hcp_dist_matrix2[i, j] = HCP(test_3D[i], train_3D[j], 'Hm')
                # hcp_dist_matrix2[i, j] = sliced_wasserstein_like_np(test_3D[i], train_3D[j], ftype="poly", degree=2, nofprojections=10)
                hcp_dist_matrix2[i, j] = mmd_imq_dist_np(test_3D[i], train_3D[j],  Cs)
        t3 = time.time()
        dt_test_train = t3 - t2
        print("Time for test-train MMD distances:", dt_test_train, "seconds")
    ...
    print("Train-train MMD time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
        min(time_train_train_list), max(time_train_train_list), sum(time_train_train_list) / len(time_train_train_list)
    ))

    print("Test-train MMD time: min = {:.3f}s, max = {:.3f}s, mean = {:.3f}s".format(
