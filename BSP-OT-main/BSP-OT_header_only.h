...
static BijectiveMatching refine_with_topm_candidates(
    const std::vector<BijectiveMatching>& plans,
    const cost_function& cost,
    BijectiveMatching init,
    int m = 8,
    int iters = 2
) {
    ...
    ints rslt  = init;
    ints rsltI = init.inverseMatching();
    Vec  C     = evalMappings(init, cost);

    std::vector<std::vector<int>> cands(N);
    cands.assign(N, {});

    for (int a = 0; a < N; ++a) {
        std::vector<int> ys;
        ys.reserve(plans.size());
        for (size_t pi = 0; pi < plans.size(); ++pi) ys.push_back((int)plans[pi][a]);
        std::sort(ys.begin(), ys.end());
        ys.erase(std::unique(ys.begin(), ys.end()), ys.end());

        std::vector<std::pair<scalar,int>> scored;
        scored.reserve(ys.size());
        for (size_t t = 0; t < ys.size(); ++t) {
            int y = ys[t];
            scored.push_back(std::make_pair(cost(a, y), y));
        }

        if ((int)scored.size() > m) {
            auto mid = scored.begin() + m;
            std::nth_element(scored.begin(), mid, scored.end(),
                [](const std::pair<scalar,int>& p1, const std::pair<scalar,int>& p2) {
        ...
        cands[a].reserve(scored.size());
        for (size_t t = 0; t < scored.size(); ++t) cands[a].push_back(scored[t].second);
    }
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        for (int a = 0; a < N; ++a) {
            ...
            for (size_t t = 0; t < cands[a].size(); ++t) {
                int bp = cands[a][t];
                if (bp == b) continue;

                int ap = rsltI[bp];
                if (ap < 0 || ap == a) continue;

                scalar old_cost = C[a] + C[ap];

                scalar cabp = cost(a, bp);
                if (cabp >= old_cost) continue;

                scalar capb = cost(ap, b);
                scalar new_cost = cabp + capb;
    ...
    return BijectiveMatching(rslt);
}


static BijectiveMatching MergePlansTopM(
    const std::vector<BijectiveMatching>& plans,
    const cost_function& cost,
    ...
    if (plans.empty()) return T0;

    // rank plans (small -> large)
    ints I = rankPlans(plans, cost);

    std::vector<BijectiveMatching> seq;
    seq.reserve(plans.size() + (T0.size() ? 1 : 0));
    for (size_t t = 0; t < I.size(); ++t) {
        int idx = (int)I[t];
        seq.push_back(plans[idx]);
    }
    if (T0.size() != 0) seq.push_back(T0);

    // merge forward: small -> large
    BijectiveMatching base_fwd = seq[0];
    for (size_t i = 1; i < seq.size(); ++i) {
        std::vector<BijectiveMatching> two;
        two.reserve(2);
        two.push_back(base_fwd);
        two.push_back(seq[i]);
        base_fwd = MergePlans(two, cost, BijectiveMatching(), cycle);
    }

    // TopM refine using original plans as candidates
    return refine_with_topm_candidates(plans, cost, base_fwd, m, iters);
}
...
inline scalar g_radial_prob = scalar(0.0);

inline void set_radial_prob(scalar p) {
    if (p < scalar(0)) p = scalar(0);
    if (p > scalar(1)) p = scalar(1);
    g_radial_prob = p;
}

inline scalar get_radial_prob() {
    return g_radial_prob;
}
...
inline bool use_radial_split() const {
    const scalar p = get_radial_prob();     // 读全局
    if (p <= scalar(0)) return false;
    if (p >= scalar(1)) return true;
    const int r = randint(0, 1000000);
    return r < int(p * scalar(1000000));
}

    // 只算当前子块的均值（比 computeMoments 轻）
    static Vector<D> computeMeanOnly(const Pts& mat, const ids& I, int b, int e) {
        Vector<D> mean = Vector<D>::Zero(mat.rows());
        for (int k = b; k < e; ++k) mean += mat.col(I[k].id);
        mean /= scalar(e - b);
        return mean;
    }

    // 用“径向距离(平方)”作为 dot 分数，然后 nth_element 切分
    void BSP_radial(ids& idA, ids& idB, int beg, int end, int pivot,
                const Vector<D>& centerA, const Vector<D>& centerB) {
    for (auto i : range(beg, end)) {
        const int ia = idA[i].id;
        const int ib = idB[i].id;
        idA[i].dot = (A.col(ia) - centerA).squaredNorm();
        idB[i].dot = (B.col(ib) - centerB).squaredNorm();
    }
...
//        BSP(idA,idB,beg,end,pivot,d);
        ...
        if (use_radial_split()) {
    const auto centerA = computeMeanOnly(A, idA, beg, end);
    const auto centerB = computeMeanOnly(B, idB, beg, end);
    BSP_radial(idA, idB, beg, end, pivot, centerA, centerB);
} else {
    const Vector<D> d = hasSliceBasis ? sliceBasis.col(height % dim)
                                      : getSlice(idA, idB, beg, end);
    BSP(idA, idB, beg, end, pivot, d);
}
        //////////////////////////////////////////////////////////////////////////////////
...
template<int D>
class BijectiveBSPMatchingGR {
public:
    using TransportPlan = ints;
    using Pts   = BSPOT::Points<D>;
    using Vec   = std::conditional_t<(D == -1), Eigen::VectorXd, Vector<D>>;
    using DynMat = Eigen::Matrix<scalar, Eigen::Dynamic, Eigen::Dynamic>;
    using DynVec = Eigen::Matrix<scalar, Eigen::Dynamic, 1>;

    BijectiveBSPMatchingGR(const Pts& A_, const Pts& B_) : A(A_), B(B_) {
        this->dim = static_cast<int>(A.rows());
    }

    void setGuided(int guided_depth_, int n_dirs_, int k_cross_, int R_) {
        this->guided_depth = guided_depth_;
        this->n_dirs = n_dirs_;
        this->k_cross = k_cross_;
        this->R = R_;
    }

    BijectiveMatching computeMatching(bool random_pivot_ = true) {
        this->random_pivot = random_pivot_;

        ids idA(static_cast<int>(A.cols())), idB(static_cast<int>(B.cols()));
        for (int i = 0; i < static_cast<int>(A.cols()); ++i) {
            idA[i].id = i;
            idB[i].id = i;
        }

        this->partialBSPOT_GR(idA, idB, 0, static_cast<int>(A.cols()), 0);

        ints plan = TransportPlan(static_cast<int>(A.cols()));
        for (int i = 0; i < static_cast<int>(A.cols()); ++i) plan[idA[i].id] = idB[i].id;
        return BijectiveMatching(plan);
    }
...
private:
    ...
    int max_scan = 2048;
    scalar eps_box  = scalar(1e-12);
    scalar eps_dist = scalar(1e-12);

    struct dot_id {
        scalar dot{};
        int id{};
        bool operator<(const dot_id& other) const { return dot < other.dot; }
    };
    using ids = std::vector<dot_id>;
...
private:
    Vec sample_dir() { return sampleUnitGaussian<D>(this->dim); }

    static scalar median_of(std::vector<scalar>& v) {
    ...
    static int ceil_neg_log2(scalar dist01, scalar eps) {
        const scalar x = dist01 + eps;
        const scalar ell_real = -std::log(x) / scalar(std::log(2.0));
        return static_cast<int>(std::ceil(ell_real));
    }

    EdgeCache precompute_edges(const ids& idA, const ids& idB, int beg, int end) {
    EdgeCache cache;
    const int N = end - beg;
    cache.N = N;
    if (N <= 1) return cache;
    ...
    // 1) X∪Y 的包围盒
    DynVec mn = DynVec::Constant(this->dim,  std::numeric_limits<scalar>::infinity());
    DynVec mx = DynVec::Constant(this->dim, -std::numeric_limits<scalar>::infinity());

    for (int t = 0; t < N; ++t) {
        const int ia = idA[beg + t].id;
        const int ib = idB[beg + t].id;
        const DynVec xa = this->A.col(ia);
        const DynVec yb = this->B.col(ib);
        mn = mn.cwiseMin(xa).cwiseMin(yb);
        mx = mx.cwiseMax(xa).cwiseMax(yb);
    }

    const DynVec rg = (mx - mn).array() + this->eps_box;

    DynMat XA01(this->dim, N), YB01(this->dim, N);
    for (int t = 0; t < N; ++t) {
        const int ia = idA[beg + t].id;
        const int ib = idB[beg + t].id;
        XA01.col(t) = (this->A.col(ia) - mn).cwiseQuotient(rg);
        YB01.col(t) = (this->B.col(ib) - mn).cwiseQuotient(rg);
    }

    // 2) B 候选集合：全量 or 抽样（允许重复）
    std::vector<int> candB;
    const int want = std::min(N, this->max_scan);
    candB.reserve(want);

    if (N <= this->max_scan) {
        for (int t = 0; t < N; ++t) candB.push_back(t);
    } else {
        for (int s = 0; s < want; ++s) candB.push_back(randint(0, N - 1));
    }

    const int k = std::max(1, std::min(this->k_cross, static_cast<int>(candB.size())));

    cache.EA.reserve(static_cast<size_t>(N) * static_cast<size_t>(k));
    cache.EB.reserve(static_cast<size_t>(N) * static_cast<size_t>(k));
    cache.ell.reserve(static_cast<size_t>(N) * static_cast<size_t>(k));

    using Pair = std::pair<scalar, int>;
    auto cmp = [](const Pair& p, const Pair& q) { return p.first < q.first; };

    // 3) 对每个 A 点：找 top-k
    for (int ai = 0; ai < N; ++ai) {
        std::vector<Pair> buf;
        buf.reserve(candB.size());

        const DynVec xa = XA01.col(ai);
        for (int idx = 0; idx < static_cast<int>(candB.size()); ++idx) {
            const int bj = candB[idx];
            const scalar d2 = (xa - YB01.col(bj)).squaredNorm();
            buf.emplace_back(d2, bj);
        }

        const int take = std::min(k, static_cast<int>(buf.size()));
        if (take <= 0) continue;

        if (static_cast<int>(buf.size()) > take) {
            std::nth_element(buf.begin(), buf.begin() + (take - 1), buf.end(), cmp);
        }

        for (int t = 0; t < take; ++t) {
            const int bj = buf[t].second;
            const scalar dist = std::sqrt(std::max(buf[t].first, scalar(0))) + this->eps_dist;
            const int ell = static_cast<int>(std::ceil(-std::log(dist) / scalar(std::log(2.0))));

            cache.EA.push_back(ai);
            cache.EB.push_back(bj);
            cache.ell.push_back(ell);
        }
    }
    ...
    scalar score_given_edges(
        const ids& idA, const ids& idB,
        int beg, int end, int height,
        const Vec& d,
        ...
        std::vector<scalar> projA(N), projB(N);
        for (int t = 0; t < N; ++t) {
            const int ia = idA[beg + t].id;
            const int ib = idB[beg + t].id;
            projA[t] = d.dot(this->A.col(ia));
            projB[t] = d.dot(this->B.col(ib));
        }

        std::vector<scalar> tmpA = projA;
        std::vector<scalar> tmpB = projB;
        const scalar medA = median_of(tmpA);
        const scalar medB = median_of(tmpB);

        std::vector<uint8_t> sideA(N), sideB(N);
        for (int t = 0; t < N; ++t) {
            sideA[t] = (projA[t] >= medA) ? uint8_t(1) : uint8_t(0);
            sideB[t] = (projB[t] >= medB) ? uint8_t(1) : uint8_t(0);
        }

        const int HR = height + this->R;
        scalar score = 0;

        const int M = static_cast<int>(cache.EA.size());
        for (int e = 0; e < M; ++e) {
            const int ai = cache.EA[e];
            const int bj = cache.EB[e];
            const bool cut = ((sideA[ai] ^ sideB[bj]) != 0);
            if (!cut) continue;

            const int over = cache.ell[e] - HR;
            if (over > 0) score += scalar(over);
        }

        return score;
    }

    Vec choose_dir_GR(const ids& idA, const ids& idB, int beg, int end, int height) {
        const EdgeCache cache = this->precompute_edges(idA, idB, beg, end);

        Vec best = this->sample_dir();
        scalar bestScore = std::numeric_limits<scalar>::infinity();

        for (int t = 0; t < this->n_dirs; ++t) {
            Vec d = this->sample_dir();
            scalar sc = this->score_given_edges(idA, idB, beg, end, height, d, cache);
            if (sc < bestScore) { bestScore = sc; best = d; }
        }
        return best;
    }

    void BSP(ids& idA, ids& idB, int beg, int end, int pivot, const Vec& d) {
        for (int i = beg; i < end; ++i) {
            idA[i].dot = d.dot(this->A.col(idA[i].id));
            idB[i].dot = d.dot(this->B.col(idB[i].id));
        }
        std::nth_element(idA.begin() + beg, idA.begin() + pivot, idA.begin() + end);
        std::nth_element(idB.begin() + beg, idB.begin() + pivot, idB.begin() + end);
    }

    void partialBSPOT_GR(ids& idA, ids& idB, int beg, int end, int height) {
        const int gap = end - beg;
        if (gap <= 1) return;

        const bool guided = (height < this->guided_depth);

        int pivot = 0;
        if (guided) pivot = beg + gap / 2;
        else pivot = this->random_pivot ? randint(beg + 1, end - 1) : beg + gap / 2;

        const Vec d = guided ? this->choose_dir_GR(idA, idB, beg, end, height)
                             : this->sample_dir();

        this->BSP(idA, idB, beg, end, pivot, d);

        this->partialBSPOT_GR(idA, idB, beg,   pivot, height + 1);
        this->partialBSPOT_GR(idA, idB, pivot, end,   height + 1);
    }
};
...
template<int D>
class BijectiveBSPMatchingEIG2 {
public:
    using TransportPlan = ints;
    using Pts   = BSPOT::Points<D>;
    using Vec   = std::conditional_t<(D == -1), Eigen::VectorXd, Vector<D>>;
    ...
    BijectiveBSPMatchingEIG2(const Pts& A_, const Pts& B_) : A(A_), B(B_) {
        this->dim = static_cast<int>(A.rows());
    }

    // 新接口：外层/内层都可控 + alpha/eta/eps_boundary
    void setGuided(
        int guided_depth_,
        int inner_iters_,
        int outer_iters_,
        int k_cross_,
        int R_,
        scalar alpha_ = scalar(10),
        scalar eta_   = scalar(0.7),
        scalar eps_boundary_ = scalar(1e-12)
    ) {
        this->guided_depth = guided_depth_;
        this->inner_iters  = std::max(1, inner_iters_);
        this->outer_iters  = std::max(1, outer_iters_);
        this->k_cross      = k_cross_;
        this->R            = R_;
        this->alpha        = alpha_;
        this->eta          = eta_;
        this->eps_boundary = eps_boundary_;
    }
    ...
    BijectiveMatching computeMatching(bool random_pivot_ = true) {
        ...
        this->partialBSPOT_EIG2(idA, idB, 0, static_cast<int>(A.cols()), 0);
...
private:
    ...
    scalar alpha = scalar(10);
    scalar eta   = scalar(0.7);
    scalar eps_boundary = scalar(1e-12);
    ...
    Vec coord_median_center(const Pts& P, const ids& idP, int beg, int end) const {
        const int N = end - beg;
        Vec c;
        if constexpr (D == -1) c = Vec::Zero(this->dim);
        else c = Vec::Zero();

        if (N <= 0) return c;

        std::vector<scalar> buf(static_cast<size_t>(N));
        for (int r = 0; r < this->dim; ++r) {
            for (int t = 0; t < N; ++t) {
                const int ip = idP[beg + t].id;
                buf[t] = P(r, ip);
            }
            c[r] = median_of(buf);
        }
    ...
    // 你原来的 precompute_edges（我只把 vmin/vmax 命名固定一下）
    EdgeCache precompute_edges(const ids& idA, const ids& idB, int beg, int end) {
        EdgeCache cache;
        const int N = end - beg;
        cache.N = N;
        ...
        DynVec vmin = DynVec::Constant(this->dim,  std::numeric_limits<scalar>::infinity());
        DynVec vmax = DynVec::Constant(this->dim, -std::numeric_limits<scalar>::infinity());
        ...
        for (int t = 0; t < N; ++t) {
            ...
            const DynVec xa = this->A.col(ia);
            const DynVec yb = this->B.col(ib);
            vmin = vmin.cwiseMin(xa).cwiseMin(yb);
            vmax = vmax.cwiseMax(xa).cwiseMax(yb);
        }

        const DynVec rg = (vmax - vmin).array() + this->eps_box;

        DynMat XA01(this->dim, N), YB01(this->dim, N);
        for (int t = 0; t < N; ++t) {
            const int ia = idA[beg + t].id;
            const int ib = idB[beg + t].id;
            XA01.col(t) = (this->A.col(ia) - vmin).cwiseQuotient(rg);
            YB01.col(t) = (this->B.col(ib) - vmin).cwiseQuotient(rg);
        }
        ...
        for (int ai = 0; ai < N; ++ai) {
            ...
            const DynVec xa = XA01.col(ai);
            for (int idx = 0; idx < (int)candB.size(); ++idx) {
                const int bj = candB[idx];
                const scalar d2 = (xa - YB01.col(bj)).squaredNorm();
                buf.emplace_back(d2, bj);
            }

            const int take = std::min(k, (int)buf.size());
            if (take <= 0) continue;

            if ((int)buf.size() > take) {
                std::nth_element(buf.begin(), buf.begin() + (take - 1), buf.end(), cmp);
            }
            ...
            for (int t = 0; t < take; ++t) {
                const int bj = buf[t].second;
                const scalar dist = std::sqrt(std::max(buf[t].first, scalar(0))) + this->eps_dist;
                const int ell = (int)std::ceil(-std::log(dist) / scalar(std::log(2.0)));
    ...
    // ====== 关键：外层更新阈值，内层 power iteration（阈值固定）======
    Vec choose_dir_EIG2(const ids& idA, const ids& idB, int beg, int end, int height) {
        const EdgeCache cache = this->precompute_edges(idA, idB, beg, end);
        if (cache.N <= 1) {
            Vec d = this->sample_dir();
            scalar dn = d.norm();
        ...
        const Vec cA = this->coord_median_center(this->A, idA, beg, end);
        const Vec cB = this->coord_median_center(this->B, idB, beg, end);
        const int HR = height + this->R;
        ...
        const int N = cache.N;
        std::vector<scalar> projA(N), projB(N), tmp;
        std::vector<uint8_t> sideA(N), sideB(N);

        // 外层：更新阈值 & 活跃边集
        for (int out = 0; out < this->outer_iters; ++out) {

            // 1) 用当前 d 计算投影，并取 median 阈值（固定在本轮内层）
            for (int t = 0; t < N; ++t) {
                const int ia = idA[beg + t].id;
                const int ib = idB[beg + t].id;
                projA[t] = d.dot(this->A.col(ia));
            ...
            tmp = projA;
            const scalar tA = median_of(tmp);
            tmp = projB;
            const scalar tB = median_of(tmp);

            for (int t = 0; t < N; ++t) {
                sideA[t] = (projA[t] >= tA) ? uint8_t(1) : uint8_t(0);
                sideB[t] = (projB[t] >= tB) ? uint8_t(1) : uint8_t(0);
            }

            // 2) 内层：对“固定阈值下的固定权重目标”做 power iteration
            for (int it = 0; it < this->inner_iters; ++it) {
                Vec Sd;
                if constexpr (D == -1) Sd = Vec::Zero(this->dim);
                else Sd = Vec::Zero();

                const int M = (int)cache.EA.size();
                for (int e = 0; e < M; ++e) {
                    const int over0 = cache.ell[e] - HR;
                    if (over0 <= 0) continue;
                    ...
                    // 只针对“当前阈值下会被切开”的边加权
                    if ( ((sideA[ai] ^ sideB[bj]) == 0) ) continue;

                    const int ia = idA[beg + ai].id;
                    const int ib = idB[beg + bj].id;

                    // 方案1：边界增强：越靠近阈值越重要
                    const scalar da = std::abs(projA[ai] - tA);
                    const scalar db = std::abs(projB[bj] - tB);
                    const scalar dist_to_boundary = da + db;

                    // boundary_boost in (0,1]
                    const scalar boundary_boost =
                        std::exp(-(dist_to_boundary / (this->eta + this->eps_boundary)));

                    // 总权重：over * (1 + alpha * boost)
                    const scalar w = (scalar(over0)) * (scalar(1) + this->alpha * boundary_boost);

                    const Vec u = (this->A.col(ia) - cA);
                    const Vec v = (this->B.col(ib) - cB);

                    const scalar vd = v.dot(d);
                    const scalar ud = u.dot(d);

                    Sd += (scalar(0.5) * w) * (u * vd + v * ud);
                }

                const scalar nrm = Sd.norm();
                if (nrm <= scalar(1e-18)) break;
                d = Sd / nrm;
            }
        }
    ...
    void partialBSPOT_EIG2(ids& idA, ids& idB, int beg, int end, int height) {
        const int gap = end - beg;
        if (gap <= 1) return;
        ...
        const Vec d = guided ? this->choose_dir_EIG2(idA, idB, beg, end, height)
                             : this->sample_dir();

        this->BSP(idA, idB, beg, end, pivot, d);

        this->partialBSPOT_EIG2(idA, idB, beg,   pivot, height + 1);
        this->partialBSPOT_EIG2(idA, idB, pivot, end,   height + 1);
    }
};
...
//////////////////////////////////////////////////////////////////////////////////////////////////
template<int dim>
BijectiveMatching computeBijectiveBSPOT_GR(
    const Points<dim>& A,
    const Points<dim>& B,
    int nb_plans,
    const cost_function& cost,
    ...
    int k_cross,
    int R,
    BijectiveMatching T = BijectiveMatching()
) {
    std::vector<BijectiveMatching> plans(nb_plans);

#pragma omp parallel for
    for (int i = 0; i < nb_plans; ++i) {
        BijectiveBSPMatchingGR<dim> BSP(A, B);
        BSP.setGuided(guided_depth, n_dirs, k_cross, R);
        plans[i] = BSP.computeMatching();
    }
...
//////////////////////////////////////////////////////////////////////////////////////////////////
template<int dim>
BijectiveMatching computeBijectiveBSPOT_EIG2(
    const Points<dim>& A,
    const Points<dim>& B,
    int nb_plans,
    const cost_function& cost,
...
#pragma omp parallel for
    for (int i = 0; i < nb_plans; ++i) {
        BSPOT::BijectiveBSPMatchingEIG2<dim> BSP(A, B);
        BSP.setGuided(guided_depth, inner_iters, outer_iters, k_cross, R, alpha, eta, eps_boundary);
        plans[i] = BSP.computeMatching();
    }
